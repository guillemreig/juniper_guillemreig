...
function logSizes(dirPath) {
    ...
    return readdir(dirPath, { withFileTypes: true }).then((dirContents) => {
        for (let item of dirContents) {
            let filePath = path.join(dirPath, item.name);
            if (item.isFile()) {
                promisesArr.push(
                    stat(filePath).then((fileInfo) => {
...
// const logSizesPromise = util.promisify(logSizes);

logSizes(filesDirPath);

// Promise.all([logSizes(filesDirPath)])
//     .then((results) => {
//         console.log(results);
//         console.log("DONE!");
//     })
