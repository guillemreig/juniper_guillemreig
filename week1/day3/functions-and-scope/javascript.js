...
function getTotaler() {
    ...
    totalSum = 0; //We use Undeclared Assignment to make a global variable that will keep (For some reason it also works with 'var' declaration. Shouldn't it be a local variable then, inaccessible to the new 'totaler' function?)
    return function (num) {
        totalSum += num;
        return totalSum;
    };
